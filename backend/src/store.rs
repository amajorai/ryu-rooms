...
impl RoomStore {
    ...
    pub async fn append_user_turn(
        ...
        if let Some(key) = idempotency_key.as_deref() {
            ...
            if let Some((run_id, previous_text)) = existing {
                ...
                drop(connection);
                let snapshot = self
                    .snapshot(&session.room_id)
                    .await?
                    .ok_or(StoreError::NotFound)?;
                let run = self
                    .run_by_id_sync(&session.room_id, &run_id)?
                    .ok_or(StoreError::NotFound)?;
                let user_message = snapshot
                    .messages
                    .iter()
                    .rev()
                    .find(|message| message.role == MessageRole::User && message.text == text)
                    .cloned()
                    .ok_or(StoreError::NotFound)?;
        ...
        transaction.execute(
            ...
            params![run_id, session.room_id, request_id, idempotency_key, text, now],
        )?;
        transaction.execute(
            "UPDATE rooms SET status = 'running', updated_at = ?2 WHERE id = ?1",
            params![session.room_id, now],
        )?;
        transaction.commit()?;
        drop(connection);
        let snapshot = self
            ...
            .await?
            .ok_or(StoreError::NotFound)?;
        let run = snapshot.current_run.clone().ok_or(StoreError::NotFound)?;
        let user_message = snapshot
            .messages
            .iter()
            .rev()
    ...
    pub async fn finish_run(
        ...
        if !matches!(current_status.as_str(), "queued" | "running") {
            transaction.commit()?;
            return snapshot_from_connection(&connection, &room_id)?.ok_or(StoreError::NotFound);
        }
        let now = Utc::now().to_rfc3339();
