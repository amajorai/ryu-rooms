...
export function RoomWorkbench({
	...
	const [confirmation, setConfirmation] = useState<"close" | "revoke" | null>(
		null
	);
			...
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							{confirmation === "close"
								? "This closes the room and revokes its current guest sessions."
								: "This revokes the guest invite and existing guest sessions."}
						</AlertDialogDescription>
					</AlertDialogHeader>
					{error ? (
					...
					<AlertDialogFooter>
						...
						<Button
							disabled={busy}
							onClick={() =>
								void (confirmation === "close" ? close() : revoke())
							}
							variant="destructive"
						>
							{busy
